...
Engine::Engine(const std::string map_file, const std::string config_file)
    : lumberjack_queue(Resource(ID_EMPTY)),
      miner_queue(Resource(ID_EMPTY)),
      farmer_queue(Resource(ID_EMPTY)),
      inventory(),
...
void Engine::spawn_producers(WorkerList &worker_list) {
    for (int i = 0; i < worker_list[NM_SMITHS]; i++) {
        Producer *p =
            new Producer(&inventory, RECIPE_SMITH, PTS_SMITH, &point_container);
        p->start();
        producers.push_back(p);
    }
    for (int i = 0; i < worker_list[NM_COOKS]; i++) {
        Producer *p =
            new Producer(&inventory, RECIPE_COOK, PTS_COOK, &point_container);
        p->start();
        producers.push_back(p);
    }
    for (int i = 0; i < worker_list[NM_CARPENTERS]; i++) {
        Producer *p = new Producer(&inventory, RECIPE_CARPENTER, PTS_CARPENTER,
                                   &point_container);
        p->start();
        producers.push_back(p);
    }
